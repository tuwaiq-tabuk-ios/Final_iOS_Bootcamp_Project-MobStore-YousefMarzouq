          ...
          if document.documentID != "ordersCount" {
            ...
            for (_, values) in data {
              ...
              for product in data["orders"] as! [[String:Any]] {
                ...
                db
                  .collection("Prodects").document(id).addSnapshotListener { snapshot, error in
...
extension CustomerOrderS : UICollectionViewDelegate,
  ...
  func collectionView(_ collectionView: UICollectionView,
    ...
    let cell = collectionCustomerOrderS.dequeueReusableCell(withReuseIdentifier: "collectionPage5", for: indexPath) as! Page5CollectionViewCell
    
    cell.imgItm.sd_setImage(with: URL(string: customerOrderS[indexPath.row].orders[0].product.image))
    
    cell.loucitonCoustmer.text = "\(customerOrderS[indexPath.row].billingAddress[0]) \(customerOrderS[indexPath.row].billingAddress[1])"
    
    
    
    cell.namburPhone.text = customerOrderS [indexPath.row].customerPhone
    
    cell.nameCastmer.text = customerOrderS[indexPath.row].customerName
    
    cell.nameOFitme.text =  customerOrderS[indexPath.row].orders[0].product.info
    
    cell.praicOFitme.text = "\(customerOrderS[indexPath.row].totalAmount) SR"
    return cell
  }
